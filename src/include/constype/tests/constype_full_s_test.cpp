#include "doctest.h"
#include <regex>
#include "constype/details/constype_full_s.hpp"
...
{
    {
        // Standard types
        char v = 5;
        MY_REQUIRE_EQ_STRING(constype_s::full(v),
        "char"
        );
        // The macro will output the exact type
        MY_REQUIRE_EQ_STRING(TNS_constype_full(v),
        "char"
        );
    }
    {
...
TEST_CASE("constype_full_r_value_references")
{
    ...
    auto output_received_type = [](auto && v) -> TypenamePair {
        return { constype_s::full<decltype(v)>(), constype_s::full(v) };
    };
 ...
 TEST_CASE("TNS_constype_full")
 {
     ...
     {
         MY_REQUIRE_EQ_STRING(
             TNS_constype_full(static_cast<char>(42)),
             "char"
          );
     }
 }
 ...
 {
     MY_REQUIRE_EQ_STRING(
         constype_s::full(static_cast<char>(1)),
         "char"
     );
     MY_REQUIRE_EQ_STRING(
         constype_s::full(static_cast<char>(1), static_cast<char>(1)),
         "char, char"
     );
     std::string tst = constype_s::full(static_cast<char>(1), "hello");
 #ifndef _MSC_VER // msvc emits "char, const char[6] const &"
     MY_REQUIRE_EQ_STRING(
         constype_s::full(static_cast<char>(1), "hello"),
         "char, const char *"
     );
 #endif
     {
         ...
         char &v2 = a;
         const char& c = a;
          MY_REQUIRE_EQ_STRING(constype_s::full('0', '0', c),
...
void check_matches(std::string const& re) {
    std::string name = constype_s::full<T>();
    std::regex regex{re};
...
void compile_time_tests() {
    RUN_ONE_TYPE_TEST_COMPILE_TIME(void, "constype_s::internal::TupleTypeHolder<void>");
    RUN_ONE_TYPE_TEST_COMPILE_TIME(char, "constype_s::internal::TupleTypeHolder<char>");
    ...
    RUN_ONE_TYPE_TEST_COMPILE_TIME(const char, "constype_s::internal::TupleTypeHolder<const char>");
    RUN_ONE_TYPE_TEST_COMPILE_TIME(char const, "constype_s::internal::TupleTypeHolder<const char>");
}
