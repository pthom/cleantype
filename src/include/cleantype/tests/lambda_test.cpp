...
TEST_CASE("log_type_lambda_clean")
{
    ...
    }
    {
        ...
        test_one_lambda(
            f, "[lambda: (std::string const &, std::string const &) -> std::string const] f");
    }
    {
        int c = 5;
        auto f = [](int a, int b) {
            return std::pair<int, double>(a + b, cos(a + static_cast<double>(b)));
        };
...
{
    {
        auto lambda = [](auto v) { return v + 42; };
        REQUIRE_EQ(cleantype::lambda_full<int>(lambda), "lambda: (int) -> int");
    }
    {
        auto lambda = [](int v) { return v + 42; };
...
{
    {
        ...
        REQUIRE_EQ(cleantype::lambda<std::string>(make_repeat_vector, true),
        ...
        // REQUIRE_EQ(
        //     full_type,
        //     "lambda: (std::__1::basic_string<char>, unsigned int) ->
